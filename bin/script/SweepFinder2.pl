...
foreach my $pop (keys %pop_name){
  foreach my $thischr (@ar_chrs){
    ...
    $pm -> start and next;
    my $output = `cd $outdir/$pop/$pop\_sf2_wd/$thischr; $hs_param{'sweepfinder_path'}  -sg $hs_param{'grid_size'} $outdir/$pop/$pop.$thischr.SF2.input $outdir/$pop/$pop.$thischr.SF2out`;
...
foreach my $pop (keys %pop_name){
    #`run R script drawing the plot `;
    `Rscript $visSFout  $pop  $outdir/$pop/  $f_ref_size  `;
}