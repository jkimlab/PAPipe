...
#2. export PCs
`$hs_param{"plink2"}  -pfile $outdir/plink2 --read-freq $outdir/plink2.afreq --pca allele-wts --out $outdir/ref_pcs &> $outdir/exportPC.log`;
...
my $endpc = 5+$hs_param{'pca_num'};
my $score_col_nums = "6-".$endpc;
#3. project onto those PCs with 
`$hs_param{"plink2"}  -pfile $outdir/plink2 --out $outdir/new_projection --read-freq $outdir/plink2.afreq --score $outdir/ref_pcs.eigenvec.allele 2 5 header-read no-mean-imputation variance-standardize  --score-col-nums $score_col_nums `;
print STDERR "$hs_param{'plink2'}  -pfile $outdir/plink2 --out new_projection --read-freq $outdir/plink2.afreq --score $outdir/ref_pcs.eigenvec.allele 2 5 header-read no-mean-imputation variance-standardize  --score-col-nums $score_col_nums\n";;


#4 visualization
#group setting 
`cut -f1 $outdir/new_projection.sscore  | cut -f1 -d"_"  > $outdir/group`;
`Rscript $Bin/visPCA_projection.R $hs_param{'title'}  $hs_param{'pca_num'} $outdir/ref_pcs.eigenval  $outdir/ref_pcs.eigenvec  $outdir/group $hs_param{'variance'} $outdir `;
print STDERR "Rscript $Bin/visPCA_projection.R $hs_param{'title'}  $hs_param{'pca_num'} $outdir/ref_pcs.eigenval  $outdir/ref_pcs.eigenvec  $outdir/group $hs_param{'variance'} $outdir\n";
